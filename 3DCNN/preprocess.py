...
COLUMNS = ['P3','C3','F3','Fz','F4','C4','P4','Cz','CM','A1','Fp1','Fp2','T3','T5','O1','O2','X3','X2','F7','F8','X1','A2','T6','T4']
HERTZ = 300


def preprocess_file(filename, input_directory, output_directory):
    data = np.loadtxt(input_directory + '/' + filename, delimiter=',', skiprows=1, usecols=[*range(0, 25)])
    times = data[:,0]
    ...
    np.savetxt(name, np.transpose(raw.get_data()), delimiter=",", header=header, comments="")
...
preprocess_files_in_directory('_data_active_unprocessed', '_data_active_preprocessed')
preprocess_files_in_directory('_data_passive_unprocessed', '_data_passive_preprocessed')